package notuserfriendly.mapatlasesrecut.map_collection;
...
public class EmptyMaps {
    ...
    public List<Component> getTooltips(int fullSize) {
        ...
        if (maxMapCount != -1 && fullSize + size >= maxMapCount) {
            return List.of(Component.translatable("item.map_atlases_recut.atlas.tooltip_full", "", null)
        ...
        for (var entry : maps.entrySet()) {
            ...
            if (hasNonVanilla) {
                tooltips.add(Component.translatable("item.map_atlases_recut.atlas.tooltip_empty_type", type.getName(), empties).withStyle(ChatFormatting.GRAY));
            } else {
                tooltips.add(Component.translatable("item.map_atlases_recut.atlas.tooltip_empty", empties).withStyle(ChatFormatting.GRAY));
            }
        }
        return tooltips;
    }
