package notuserfriendly.mapatlasesrecut.client.ui;
...
import notuserfriendly.mapatlasesrecut.client.AbstractAtlasDisplay;
import notuserfriendly.mapatlasesrecut.client.Anchoring;
import notuserfriendly.mapatlasesrecut.client.MapAtlasesClient;
import notuserfriendly.mapatlasesrecut.client.screen.AtlasScreenUtils;
import notuserfriendly.mapatlasesrecut.config.MapAtlasesClientConfig;
import notuserfriendly.mapatlasesrecut.integration.moonlight.ClientMarkersRenderer;
...
import static notuserfriendly.mapatlasesrecut.client.MapAtlasesClient.MAP_HUD_BACKGROUND_TEXTURE;
...
public class MapAtlasesHUD extends AbstractAtlasDisplay {
    ...
    public void drawMapComponentCoords(GuiGraphics context, Font font, int x, int y,
        ...
        if (chunk) {
            coordsToDisplay = Component.translatable("message.map_atlases_recut.chunk_coordinates",
                    pos.getX() / 16, pos.getZ() / 16, pos.getX() % 16, pos.getZ() % 16).getString();
        } else {
            coordsToDisplay = displaysY
                    ? Component.translatable("message.map_atlases_recut.coordinates_full",
                            pos.getX(), pos.getY(), pos.getZ()).getString()
                    : Component.translatable("message.map_atlases_recut.coordinates",
