...
public class SliceBookmarkButton extends AtlasButton {
    ...
    public Tooltip createTooltip() {
        return Tooltip.create(slice.height().isEmpty() ? Component.translatable("item.map_atlases_recut.atlas.tooltip_slice_default") :
                Component.translatable("item.map_atlases_recut.atlas.tooltip_slice", slice.height().get()));
    }
    ...
    protected void renderWidget(GuiGraphics pGuiGraphics, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        if (hasMoreThan1Slice) {
            ...
            Component text = h.map(integer -> Component.literal(String.valueOf(integer)))
                    .orElseGet(() -> Component.translatable("message.map_atlases_recut.atlas.slice_default"));
