...
public class CartographyTableAtlasButton extends AbstractWidget {
    ...
    protected void renderWidget(GuiGraphics pGuiGraphics, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        if (this.menu instanceof AtlasCartographyTable at) {
            if (left) {
                pGuiGraphics.drawCenteredString(Minecraft.getInstance().font, Component.translatable("message.map_atlases_recut.map_index",
                                at.mapatlasesrecut$getSelectedMapIndex()),
                        this.getX() + 30, this.getY() + 2, -1);
            } else {
                Slice slice = at.mapatlasesrecut$getSelectedSlice();
                if (slice != null) {
                    ...
                    if (height.isPresent()) {
                        pGuiGraphics.drawString(Minecraft.getInstance().font,
                                Component.translatable("message.map_atlases_recut.slice_height", height.get()),
                                this.getX() - 52, y0 + this.getY() - 50, -1);
                    }
                }
            }
